...
This project takes the `Do Comp` functions from [FirstCombinedAttempt.cs](https://github.com/joesunley/NEA-Project/blob/master/Resources/Code%20Files/Projects/FirstCombinedAttempt.cs) and simplifies them
<br>This version is a CLI interface
...
namespace Do_Comp
{
    class Program
    {
        ...
        static Competition thisCompetition = new Competition();

        static void DoComp()
        {
            List<Player> startingPlayers = thisCompetition.startingPlayers; //Gets the initial list of players in the competition
            int numRounds = thisCompetition.rounds.Count; // the number of rounds in the competition

            for (int i = 0; i < numRounds; i++)
            {
                startingPlayers = DoRound(thisCompetition.rounds[i], startingPlayers); //Does the round for each competition, and sets the output of that round to the next list of players
            }
        }

        static List<Player> DoRound(Round round, List<Player> startingPlayers)
        {
            ...
            List<Tuple<Player, double>> returnedPlayers =  GetQualifiedPlayers(allResults, round.StartingCompetitors, perGroup); //Gets the qualifing players
        ...
        static ResultsFile GetRaceResults(IRace race)
        {
        ...
        static ResultsFile GetSpResults(SpRace race) { return new ResultsFile(); }
        static ResultsFile GetMpResults(MpRace race) { return new ResultsFile(); }
        static ResultsFile GetCpResults(CpRace race) { return new ResultsFile(); }


        static List<Tuple<Player, double>> GetQualifiedPlayers(List<ResultsFile> allResults, List<Player> startingPlayers, int perGroup)
        {
            ...
            for (int currRace = 0; currRace < allResults.Count; currRace++) // runs through each race
            {
                ...
                switch (thisCompetition.QualifyingMode)
                {
        ...
        static void SendRaceFiles(Round round, List<Player> players) { }
